// ============================================================
//  GALLARZO LLC — Rent Receipt Generator  v9.3
//  Google Apps Script
// ============================================================

var DRIVE_ROOT_FOLDER_URL = "https://drive.google.com/drive/u/0/folders/1GmWU8SfqFVV8zG5PQg7jWf-p0BowPK0L";

var SKIP_SHEETS = [
  "BOOK", "EXPENSES", "EXPENSES DETAIL", "PROFIT AND LOSS",
  "Contrators Total", "Payment Methods", "Payments Made", "Collected Payments"
];

// ============================================================
//  TENANT ADDRESS OVERRIDES
//  For tenants whose receipt should show a different address than
//  the unit they rent (e.g. a separate mailing / shipping address).
//
//  KEY   = tenant name EXACTLY as it appears in the sheet.
//          (Matching ignores capitalization and surrounding spaces.)
//  VALUE = the address to print. Format it the SAME way as your unit
//          addresses: street, then ONE comma, then city/state/ZIP, so it
//          splits cleanly into two lines on the receipt.
//          Example: "456 Oak Ave, Riverside CA 92501"
//
//  This changes ONLY the mailing/shipping address in the recipient block at
//  the top of the receipt. The "Rent Amount" row still shows the actual unit
//  being rented. To add more tenants later, add another line. Leave the
//  object empty ({}) to turn the feature off entirely.
// ============================================================
var TENANT_ADDRESS_OVERRIDES = {
  "Rodolfo Campos": "10073 McKinley St, Rancho Cucamonga CA 91730"
  // "John Smith": "456 Oak Ave, Riverside CA 92501"
};

function getAddressOverride(tenantName) {
  var key = String(tenantName || "").trim().toLowerCase();
  if (!key) return "";
  for (var k in TENANT_ADDRESS_OVERRIDES) {
    if (k.trim().toLowerCase() === key) return TENANT_ADDRESS_OVERRIDES[k];
  }
  return "";
}

// A   B            C             D              E                   F           G              H              I            J                   K          L
// Mon TENANT NAME  BASE RENT($)  EXTRA CHG($)   EXTRA CHG DESC      LATE FEE($) LATE FEE WAIVE PAYMENT METHOD PAYMENT DATE PAYMENT RECEIVED($) BALANCE($) ON TIME?
var COL_MONTH    = 0;
var COL_TENANT   = 1;
var COL_RENT     = 2;
var COL_EXTRA    = 3;
var COL_LATE_FEE = 5;
var COL_WAIVE    = 6;
var COL_METHOD   = 7;
var COL_DATE     = 8;
var COL_RECEIVED = 9;
var COL_BALANCE  = 10;
var COL_ONTIME   = 11;

var MONTH_ORDER = {
  "January":1,"February":2,"March":3,"April":4,
  "May":5,"June":6,"July":7,"August":8,
  "September":9,"October":10,"November":11,"December":12
};

var TIME_BUFFER_SECS = 60;

function getLogoBase64() {
  var parts = [];
  parts.push("");
  parts.push("");
  parts.push("");
  return parts.join("");
}


// =============================================================
//  MENU
// =============================================================
function onOpen() {
  SpreadsheetApp.getUi()
    .createMenu("Receipts")
    .addItem("Generate Receipts...", "showReceiptPanel")
    .addSeparator()
    .addItem("Diagnose (check setup)", "diagnose")
    .addToUi();
}

function showReceiptPanel() {
  var html = HtmlService.createHtmlOutput(buildSidebarHtml())
    .setTitle("Generate Rent Receipts")
    .setWidth(340);
  SpreadsheetApp.getUi().showSidebar(html);
}


// =============================================================
//  STEP 1: Load months
// =============================================================
function getAvailableMonths() {
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var sheets = ss.getSheets();
  var monthSet = {};
  for (var i = 0; i < sheets.length; i++) {
    var name = sheets[i].getName().trim();
    if (SKIP_SHEETS.indexOf(name) !== -1) continue;
    var data = sheets[i].getDataRange().getValues();
    for (var r = 0; r < data.length; r++) {
      var col0 = String(data[r][0] || "").trim();
      var m = col0.match(/^(\d+)\)\s*(\w+)/);
      if (m && MONTH_ORDER[m[2]]) monthSet[m[2]] = true;
    }
  }
  var months = Object.keys(monthSet);
  months.sort(function(a, b) { return MONTH_ORDER[a] - MONTH_ORDER[b]; });
  return months;
}


// =============================================================
//  STEP 2: Load tenants for a month
// =============================================================
function getTenantsForMonth(selectedMonth) {
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var sheets = ss.getSheets();
  var tenantSet = {};
  for (var i = 0; i < sheets.length; i++) {
    var name = sheets[i].getName().trim();
    if (SKIP_SHEETS.indexOf(name) !== -1) continue;
    var data = sheets[i].getDataRange().getValues();
    for (var r = 0; r < data.length; r++) {
      var col0   = String(data[r][0] || "").trim();
      var tenant = String(data[r][COL_TENANT] || "").trim();
      var recv   = toNum(data[r][COL_RECEIVED]);
      var m = col0.match(/^(\d+)\)\s*(\w+)/);
      if (!m || m[2] !== selectedMonth) continue;
      if (tenant && recv > 0) tenantSet[tenant] = true;
    }
  }
  var tenants = Object.keys(tenantSet);
  tenants.sort();
  return tenants;
}


// =============================================================
//  QUEUE INIT
// =============================================================
function initQueue(mode, selectedMonth, selectedTenant) {
  var ss        = SpreadsheetApp.getActiveSpreadsheet();
  var sheetYear = new Date().getFullYear();
  var tm = ss.getName().match(/20\d\d/);
  if (tm) sheetYear = parseInt(tm[0]);

  var sheets = ss.getSheets();
  var queue  = [];

  for (var i = 0; i < sheets.length; i++) {
    var sheetName = sheets[i].getName().trim();
    if (SKIP_SHEETS.indexOf(sheetName) !== -1) continue;
    var data    = sheets[i].getDataRange().getValues();
    var address = "";

    for (var r = 0; r < data.length; r++) {
      var row  = data[r];
      var col0 = String(row[0] || "").trim();

      if (String(row[2] || "").trim() === "BASE RENT ($)") {
        address = col0;
        continue;
      }

      var m = col0.match(/^(\d+)\)\s*(\w+)/);
      if (!m || m[2] !== selectedMonth) continue;

      var tenant     = String(row[COL_TENANT]   || "").trim();
      var baseRent   = toNum(row[COL_RENT]);
      var extra      = toNum(row[COL_EXTRA]);
      var lateFeeRaw = toNum(row[COL_LATE_FEE]);
      var isWaived   = String(row[COL_WAIVE] || "").trim().toLowerCase() === "waived";
      var lateFee    = isWaived ? 0 : lateFeeRaw;
      var method     = String(row[COL_METHOD]   || "").trim();
      var payDate    = row[COL_DATE];
      var received   = toNum(row[COL_RECEIVED]);
      var balance    = toNum(row[COL_BALANCE]);
      var onTime     = String(row[COL_ONTIME]   || "").trim();

      if (!tenant || received <= 0) continue;
      if (mode === "single" && tenant !== selectedTenant) continue;

      // Prior balance = balance from the PREVIOUS month's row for this tenant
      // Col K accumulates — so the prior month row shows what they owed before this month
      var currentMonthNum = MONTH_ORDER[selectedMonth];
      var priorBalance = 0;
      if (currentMonthNum > 1) {
        // Look backwards through the sheet for the previous month row for this tenant
        for (var pr = 0; pr < data.length; pr++) {
          var prow = data[pr];
          var pc0  = String(prow[0] || "").trim();
          var pm   = pc0.match(/^(\d+)\)\s*(\w+)/);
          if (!pm) continue;
          var pTenant = String(prow[COL_TENANT] || "").trim();
          if (pm[2] !== selectedMonth && pTenant === tenant) {
            // Find the row with month number = currentMonthNum - 1
            var pMonthNum = MONTH_ORDER[pm[2]];
            if (pMonthNum === currentMonthNum - 1) {
              priorBalance = toNum(prow[COL_BALANCE]);
              break;
            }
          }
        }
      }

      var isLate       = (onTime === "No");
      var totalDue     = baseRent + extra + lateFee + priorBalance;
      var nextMonthIdx = MONTH_ORDER[selectedMonth];
      var nextMonthNum = (nextMonthIdx % 12) + 1;
      var nextYear     = nextMonthNum === 1 ? sheetYear + 1 : sheetYear;
      var nextDueDate  = String(nextMonthNum).padStart(2,"0") + "/01/" + nextYear;
      var lateDueDate  = String(nextMonthNum).padStart(2,"0") + "/05/" + nextYear;
      var nextLateFee  = Math.round(baseRent * 0.10 * 100) / 100;
      var nextMonthKeys = Object.keys(MONTH_ORDER).filter(function(k){ return MONTH_ORDER[k] === nextMonthNum; });
      var nextMonthName = nextMonthKeys.length ? nextMonthKeys[0] : "Next";

      // The unit address (read from this sheet's header) is the property
      // actually being rented — it always appears in the "Rent Amount" row.
      // A per-tenant override only changes the mailing/shipping address shown
      // in the recipient block at the top of the receipt.
      var mailingAddress = getAddressOverride(tenant) || address;

      queue.push({
        propertyAddress: address,
        mailingAddress:  mailingAddress,
        sheetName:       sheetName,
        tenantName:      tenant,
        paymentDate:     fmtDate(payDate),
        rentalMonth:     selectedMonth,
        rentalYear:      sheetYear,
        rentDue:         baseRent,
        extra:           extra,
        lateFee:         lateFee,
        lateFeeRaw:      lateFeeRaw,
        isWaived:        isWaived,
        totalDue:        totalDue,
        amountPaid:      received,
        balance:         balance,
        priorBalance:    priorBalance,
        paymentMethod:   method,
        isLate:          isLate,
        nextDueDate:     nextDueDate,
        lateDueDate:     lateDueDate,
        nextMonthName:   nextMonthName,
        nextRent:        baseRent,
        nextExtra:       extra,
        nextLateFee:     nextLateFee,
        nextTotal:       baseRent + extra
      });
    }
  }

  if (queue.length === 0) {
    return { success: false, message: "No paid rows found for the selected options.", total: 0 };
  }

  PropertiesService.getScriptProperties().setProperty("RECEIPT_QUEUE", JSON.stringify({
    queue: queue, nextIndex: 0, total: queue.length,
    selectedMonth: selectedMonth, sheetYear: sheetYear,
    newCount: 0, skipCount: 0
  }));

  return { success: true, total: queue.length };
}


// =============================================================
//  PROCESS BATCH
// =============================================================
function processBatch() {
  var props = PropertiesService.getScriptProperties();
  var raw   = props.getProperty("RECEIPT_QUEUE");
  if (!raw) return { done: true, message: "No queue found.", newCount: 0, skipCount: 0 };

  var state    = JSON.parse(raw);
  var queue    = state.queue || [];
  var idx      = state.nextIndex || 0;
  var deadline = Date.now() + (360 - TIME_BUFFER_SECS) * 1000;

  var rootId         = extractId(DRIVE_ROOT_FOLDER_URL);
  var rootFolder     = DriveApp.getFolderById(rootId);
  var combinedFolder = getOrCreateFolder(rootFolder, "Receipts " + state.sheetYear);

  while (idx < queue.length && Date.now() < deadline) {
    var item = queue[idx];
    try {
      var tenantFolder = getOrCreateFolder(rootFolder, sanitizeName(item.tenantName));
      var fileName     = sanitizeName(item.tenantName) + "_" + item.rentalMonth + "_" + item.rentalYear;
      // Always replace the monthly receipt PDF if it already exists.
      // This is important after layout or balance fixes; otherwise Apps Script
      // would skip the old PDF and keep showing the previous mistake.
      var existing     = tenantFolder.getFilesByName(fileName + ".pdf");
      while (existing.hasNext()) {
        existing.next().setTrashed(true);
      }

      var htmlContent = buildReceiptHtml(item, false);
      var htmlBlob    = Utilities.newBlob(htmlContent, MimeType.HTML, fileName + ".html");
      var pdfBlob     = htmlBlob.getAs(MimeType.PDF);
      pdfBlob.setName(fileName + ".pdf");
      tenantFolder.createFile(pdfBlob);
      state.newCount++;
    } catch (e) {
      Logger.log("Error for " + item.tenantName + ": " + e.message);
    }
    idx++;
  }

  state.nextIndex = idx;

  if (idx >= queue.length) {
    var combinedMsg = "";
    try {
      var combinedFile = buildCombinedPdf(queue, state.selectedMonth, state.sheetYear, combinedFolder);
      combinedMsg = " Combined PDF saved: " + combinedFile.getName() + ".";
    } catch (e) {
      combinedMsg = " Individual PDFs were processed, but the combined PDF failed: " + e.message;
      Logger.log("Combined PDF error: " + e.message);
    }

    props.deleteProperty("RECEIPT_QUEUE");

    return {
      done: true,
      message: "Done! " + state.newCount + " receipt PDF(s) saved/replaced." + combinedMsg,
      newCount: state.newCount,
      skipCount: state.skipCount
    };
  }

  props.setProperty("RECEIPT_QUEUE", JSON.stringify(state));

  return {
    done: false,
    processed: idx,
    total: queue.length,
    message: "Processing " + idx + " of " + queue.length + "...",
    newCount: state.newCount,
    skipCount: state.skipCount
  };
}


// =============================================================
//  COMBINED PDF
// =============================================================
// The old combined version let the body keep display:flex from the single-receipt
// view. When multiple 8.5in receipts were added side-by-side in one flex row,
// Apps Script's PDF converter could render the first PDF page as blank.
// This version forces block layout and gives each receipt its own letter page.
function buildCombinedPdf(queue, month, year, folder) {
  if (!queue || queue.length === 0) {
    throw new Error("No receipts found to combine.");
  }

  var combinedName = sanitizeName(month + "_" + year + "_All_Receipts") + ".pdf";

  // Remove the previous combined PDF so the folder only keeps the newest one.
  var old = folder.getFilesByName(combinedName);
  while (old.hasNext()) {
    old.next().setTrashed(true);
  }

  var pages = [];
  for (var i = 0; i < queue.length; i++) {
    pages.push(buildReceiptHtml(queue[i], true));
  }

  var combinedPdfCss =
    '<style>' +
      '@page{size:letter;margin:0;}' +
      'html{margin:0!important;padding:0!important;background:#fff!important;}' +
      'body{' +
        'display:block!important;' +
        'justify-content:initial!important;' +
        'align-items:initial!important;' +
        'width:8.5in!important;' +
        'min-width:8.5in!important;' +
        'margin:0!important;' +
        'padding:0!important;' +
        'background:#fff!important;' +
        'overflow:visible!important;' +
      '}' +
      '.receipt-container{' +
        'width:8.5in!important;' +
        'height:11in!important;' +
        'margin:0!important;' +
        'padding:0!important;' +
        'box-shadow:none!important;' +
        'page-break-after:always!important;' +
        'break-after:page!important;' +
        'page-break-inside:avoid!important;' +
        'break-inside:avoid!important;' +
        'overflow:hidden!important;' +
      '}' +
      '.receipt-container:last-child{' +
        'page-break-after:auto!important;' +
        'break-after:auto!important;' +
      '}' +
    '</style>';

  var fullHtml =
    '<!DOCTYPE html>' +
    '<html>' +
      '<head>' +
        '<meta charset="UTF-8">' +
        '<meta name="viewport" content="width=device-width, initial-scale=1.0">' +
        getReceiptCss() +
        combinedPdfCss +
      '</head>' +
      '<body>' +
        pages.join("") +
      '</body>' +
    '</html>';

  var htmlBlob = Utilities.newBlob(
    fullHtml,
    MimeType.HTML,
    combinedName.replace(".pdf", ".html")
  );

  var pdfBlob = htmlBlob.getAs(MimeType.PDF);
  pdfBlob.setName(combinedName);

  var file = folder.createFile(pdfBlob);

  // A very tiny PDF usually means the converter returned an empty/blank result.
  // This gives you a real error instead of a false "saved" message.
  if (file.getSize && file.getSize() < 1000) {
    file.setTrashed(true);
    throw new Error("The combined PDF was created too small, likely blank. Try generating fewer receipts at once.");
  }

  return file;
}



function getMontserrat400() {
  var p = [];
  p.push("");
  p.push("");
  p.push("");
  p.push("");
  return p.join("");
}

function getMontserrat500() {
  var p = [];
  p.push("");
  p.push("");
  p.push("");
  p.push("");
  return p.join("");
}

function getMontserrat600() {
  var p = [];
  p.push("");
  p.push("");
  p.push("");
  p.push("");
  return p.join("");
}

function getMontserrat700() {
  var p = [];
  p.push("");
  p.push("");
  p.push("");
  p.push("");
  return p.join("");
}

// =============================================================
//  CSS — self-contained, uses inline SVG icons
// =============================================================
function getReceiptCss() {
  return '<style>' +
    '@font-face{font-family:"Montserrat";font-style:normal;font-weight:400;src:url("data:font/woff2;base64," + getMontserrat400() + "") format("woff2");}@font-face{font-family:"Montserrat";font-style:normal;font-weight:500;src:url("data:font/woff2;base64," + getMontserrat500() + "") format("woff2");}@font-face{font-family:"Montserrat";font-style:normal;font-weight:600;src:url("data:font/woff2;base64," + getMontserrat600() + "") format("woff2");}@font-face{font-family:"Montserrat";font-style:normal;font-weight:700;src:url("data:font/woff2;base64," + getMontserrat700() + "") format("woff2");}' +
    ':root{--navy:#0e2a47;--teal:#008374;--light-teal:#eef7f6;--gray-bg:#f8f9fa;--border-color:#e0e0e0;--text-dark:#333333;--text-gray:#666666;--red:#c82333;--green:#28a745;}' +
    '*{box-sizing:border-box;margin:0;padding:0;}' +
    'body{font-family:Montserrat,Arial,Helvetica,sans-serif;background-color:#e0e0e0;display:flex;justify-content:center;padding:20px;color:var(--text-dark);-webkit-print-color-adjust:exact;print-color-adjust:exact;}' +
    '@page{size:letter;margin:0;}' +
    '@media print{body{background-color:white;padding:0;}.receipt-container{box-shadow:none!important;}}' +
    '.receipt-container{width:8.5in;height:11in;background:white;box-shadow:0 4px 15px rgba(0,0,0,0.1);position:relative;display:flex;flex-direction:column;overflow:hidden;}' +
    '.sender-block{position:absolute;top:0.52in;left:0.59in;width:2.58in;color:var(--navy);}' +
    '.sender-logo-icon{position:absolute;left:0;top:0in;width:48px;height:48px;object-fit:contain;}' +
    '.sender-text-content{position:absolute;left:0.62in;top:0;font-size:10pt;line-height:1.2;}' +
    '.recipient-block{position:absolute;top:2.18in;left:0.58in;width:5.05in;font-size:10pt;line-height:1.16;color:var(--text-dark);}' +
    '.recipient-block strong{display:block;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;font-size:10pt;line-height:1.12;margin:0;padding:0;}' +
    '.header-right-meta{position:absolute;top:0.52in;right:0.5in;text-align:right;width:4in;}' +
    '.logo-area{display:flex;align-items:center;justify-content:flex-end;margin-bottom:10px;gap:15px;}' +
    '.logo-text{text-align:left;}' +
    '.logo-text h1{font-size:21px;font-weight:700;color:var(--navy);letter-spacing:1px;line-height:1.1;}' +
    '.logo-text span{font-size:11px;color:var(--teal);letter-spacing:2px;font-weight:600;}' +
    '.receipt-title{font-size:29px;font-weight:700;color:var(--navy);margin-bottom:10px;}' +
    '.receipt-title span{color:var(--teal);}' +
    '.content-wrapper{padding-top:3.44in;padding-left:0.5in;padding-right:0.5in;padding-bottom:0.3in;flex-grow:1;display:flex;flex-direction:column;}' +
    '.content-columns{display:flex;gap:20px;margin-bottom:15px;}' +
    '.column{border:1px solid var(--border-color);border-radius:8px;padding:15px 20px;background-color:white;}' +
    '.col-left{width:55%;}.col-right{width:45%;background-color:var(--gray-bg);}' +
    '.section-header{display:flex;align-items:center;gap:12px;margin-bottom:15px;padding-bottom:12px;border-bottom:1px solid var(--border-color);}' +
    '.section-header h3{font-size:14px;color:var(--navy);font-weight:700;}' +
    '.summary-row{display:flex;justify-content:space-between;align-items:center;gap:12px;padding:10px 0;border-bottom:1px dashed var(--border-color);font-size:13px;}' +
    '.summary-row:last-child{border-bottom:none;}' +
    '.row-label{display:flex;align-items:center;gap:12px;color:var(--text-dark);font-weight:500;min-width:0;flex:1;}' +
    '.row-label>div{min-width:0;}' +
    '.row-label small{display:block;font-size:11px;color:var(--text-gray);font-weight:400;margin-top:2px;}' +
    '.row-value{font-weight:600;flex-shrink:0;text-align:right;}' +
    '.rent-summary-row .rent-line-text{min-width:0;max-width:100%;}' +
    '.rent-summary-row .rent-row-value{margin-left:8px;}' +
    /* Rent address is always two clean lines: street on line 1, city/state/ZIP on line 2. */
    '.rent-summary-row.rent-address-two-line{height:51px;min-height:51px;max-height:51px;padding-top:6px;padding-bottom:6px;align-items:flex-start;overflow:hidden;}' +
    '.rent-summary-row.rent-address-two-line .row-label{align-items:flex-start;}' +
    '.rent-summary-row.rent-address-two-line .rent-line-text{position:relative;top:-1px;}' +
    '.rent-summary-row.rent-address-two-line .rent-address-line{display:block;max-width:2.72in;white-space:normal;line-height:1.02;margin-top:0;overflow:hidden;}' +
    '.rent-summary-row.rent-address-two-line .rent-address-line span{display:block;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}' +
    '.rent-summary-row.rent-address-two-line .rent-row-value{padding-top:0;position:relative;top:-2px;}' +
    '.text-red{color:var(--red);}.text-green{color:var(--green);}' +
    '.totals-section{margin-top:10px;}' +
    '.total-row{display:flex;justify-content:space-between;align-items:center;padding:10px 0;font-size:14px;font-weight:700;}' +
    '.amount-paid-box{background-color:#eef7f6;padding:10px 12px;border-radius:6px;margin:4px 0;}' +
    '.balance-ok-box{background-color:#f0f9f0;padding:10px 12px;border-radius:6px;}' +
    '.balance-owed-box{background-color:#fdecea;padding:10px 12px;border-radius:6px;}' +
    '.detail-item{display:flex;gap:12px;margin-bottom:15px;align-items:flex-start;}' +
    '.detail-item:last-child{margin-bottom:0;padding-top:12px;border-top:1px solid var(--border-color);}' +
    '.detail-text label{display:block;font-size:11px;color:var(--text-gray);margin-bottom:3px;}' +
    '.detail-text p{font-size:13px;font-weight:600;color:var(--navy);}' +
    '.receipt-confirmation{display:flex;align-items:center;gap:10px;font-size:13px;color:var(--text-gray);margin-bottom:15px;}' +
    '.next-month-title{font-size:14px;color:var(--navy);font-weight:700;margin-bottom:8px;}' +
    '.next-month-box{border:1px solid var(--border-color);border-radius:8px;padding:15px;display:flex;flex-direction:column;gap:12px;}' +
    '.next-month-grid{display:flex;justify-content:space-between;text-align:center;}' +
    '.grid-item{flex:1;border-right:1px solid var(--border-color);padding:0 5px;}' +
    '.grid-item:last-child{border-right:none;}' +
    '.grid-item h4{font-size:11px;color:var(--text-gray);font-weight:500;margin:6px 0 4px;}' +
    '.grid-item p{font-size:14px;font-weight:600;color:var(--navy);}' +
    '.grid-item.total-due p{font-size:16px;font-weight:700;}' +
    '.late-fee-alert{background-color:#f0f4f8;border-radius:6px;padding:8px;display:flex;align-items:center;gap:8px;font-size:12px;color:var(--navy);}' +
    '.footer{margin-top:auto;border-top:1px solid var(--border-color);padding-top:15px;display:flex;flex-direction:column;align-items:center;gap:10px;font-size:12px;color:var(--text-gray);}' +
    '.footer-line{display:flex;align-items:center;justify-content:center;gap:10px;line-height:1.4;}' +
    '.footer-support-icon{width:26px;height:26px;background-color:var(--light-teal);border-radius:50%;display:flex;align-items:center;justify-content:center;flex-shrink:0;}' +
    '.footer-links{display:flex;align-items:center;gap:20px;}' +
    '.footer-link{display:flex;align-items:center;gap:6px;}' +
    '</style>';
}


// =============================================================
//  BUILD RECEIPT HTML — inline SVG icons, matches v2.html exactly
// =============================================================
function buildReceiptHtml(d, pageOnly) {
  var lc    = d.propertyAddress.lastIndexOf(",");
  var addr1 = lc !== -1 ? d.propertyAddress.substring(0, lc).trim() : d.propertyAddress.trim();
  var addr2 = lc !== -1 ? d.propertyAddress.substring(lc + 1).trim() : "";

  // Mailing/shipping address for the top recipient block. Falls back to the
  // unit address when no override is set, so non-override tenants are unchanged.
  var mAddr  = d.mailingAddress || d.propertyAddress;
  var mlc    = mAddr.lastIndexOf(",");
  var maddr1 = mlc !== -1 ? mAddr.substring(0, mlc).trim() : mAddr.trim();
  var maddr2 = mlc !== -1 ? mAddr.substring(mlc + 1).trim() : "";

  var leasePeriod  = d.rentalMonth + " 1, " + d.rentalYear + " - " + d.rentalMonth + " " + lastDayOfMonth(d.rentalMonth, d.rentalYear) + ", " + d.rentalYear;
  var notes        = d.isLate ? "Late payment recorded for this month." : "Thank you for your on-time payment!";

  // Current balance is calculated from this receipt so overpayments stay negative as credits.
  // Example: rent due $1,915.00 and paid $1,920.00 = -$5.00 credit.
  // This avoids spreadsheet balance formulas that may return a positive absolute value.
  var currentBalance    = roundMoney(d.totalDue - d.amountPaid);
  var hasBalance        = currentBalance > 0;
  var hasCredit         = currentBalance < 0;
  var balanceLabel      = hasBalance ? "REMAINING BALANCE" : (hasCredit ? "CREDIT BALANCE" : "BALANCE DUE");
  var balanceClass      = hasBalance ? "text-red" : "text-green";
  var balanceBox        = hasBalance ? "balance-owed-box" : "balance-ok-box";
  var balanceDisplay    = fmtMoney(currentBalance);
  var nextBalanceLabel  = hasCredit ? "Credit" : "Balance";
  var nextBalanceClass  = hasBalance ? "text-red" : (hasCredit ? "text-green" : "");
  var nextBalanceDisplay = fmtMoney(currentBalance);

  var nextLateFeeDisplay = d.isLate ? d.nextLateFee : 0;
  var nextTotalDisplay   = Math.max(0, roundMoney(d.nextRent + d.nextExtra + currentBalance));
  var tenantNameStyle    = recipientNameStyle(d.tenantName);
  // Keep the Rent Amount address in a clean, consistent two-line format:
  // line 1 = street address, line 2 = city/state/ZIP. The row height stays fixed
  // so the Account Summary box and the other rows do not move.
  var rentAddressHtml    = '<span>' + escHtml(addr1) + '</span><span>' + (addr2 ? escHtml(addr2) : '&nbsp;') + '</span>';
  var rentSummaryClass   = 'summary-row rent-summary-row rent-address-two-line';
  var logoSrc = "data:image/png;base64," + getLogoBase64();

  // ── Inline SVG helpers ─────────────────────────────────────────
  // Badge: colored circle with white SVG icon inside (matches .section-header i / .row-label i from v2.html)
  function badge(svgPath, vb, bg, sz, iSz) {
    return '<div style="background:' + bg + ';width:' + sz + ';height:' + sz + ';border-radius:50%;display:flex;align-items:center;justify-content:center;flex-shrink:0;">' +
           '<svg xmlns="http://www.w3.org/2000/svg" viewBox="' + vb + '" width="' + iSz + '" height="' + iSz + '" fill="white"><path d="' + svgPath + '"/></svg></div>';
  }
  // Plain inline SVG (for detail items, footer, confirmation)
  function icon(svgPath, vb, color, sz) {
    return '<svg xmlns="http://www.w3.org/2000/svg" viewBox="' + vb + '" width="' + sz + '" height="' + sz + '" fill="' + color + '" style="flex-shrink:0;"><path d="' + svgPath + '"/></svg>';
  }

  // Icon definitions
  var iDollar    = badge("M160 0c17.7 0 32 14.3 32 32V67.7c1.6 .2 3.1 .4 4.7 .7c.4 .1 .7 .1 1.1 .2l48 8.8c17.4 3.2 28.9 19.9 25.7 37.2s-19.9 28.9-37.2 25.7l-47.5-8.7c-31.3-4.6-58.9-1.5-78.3 6.2s-27.2 18.3-29 28.1c-2 10.7-.5 16.7 1.2 20.4c1.8 3.9 5.5 8.3 12.8 13.2c16.3 10.7 41.3 17.7 73.7 26.3l2.9 .8c28.6 7.6 63.6 16.8 89.6 33.8c14.2 9.3 27.6 21.9 35.9 39.5c8.5 17.9 10.3 37.9 6.4 59.2c-6.9 38-33.1 63.4-65.6 76.7c-13.7 5.6-28.6 9.2-44.4 11V480c0 17.7-14.3 32-32 32s-32-14.3-32-32V445.1c-.4-.1-.9-.1-1.3-.2l-.2 0 0 0c-24.4-3.8-64.5-14.3-91.5-26.3c-16.1-7.2-23.4-26.1-16.2-42.2s26.1-23.4 42.2-16.2c20.9 9.3 55.3 18.5 75.2 21.6c31.9 4.7 58.2 2 76-5.3c16.9-6.9 24.6-16.9 26.8-28.9c1.9-10.6 .4-16.7-1.3-20.4c-1.9-4-5.6-8.4-13-13.3c-16.4-10.7-41.5-17.7-74-26.3l-2.8-.7 0 0C119.4 279.3 84.4 270 58.4 253c-14.2-9.3-27.5-22-35.8-39.6c-8.4-17.9-10.1-37.9-6.1-59.2C23.7 116 52.3 91.2 84.8 78.3c13.3-5.3 27.9-8.9 43.2-11V32c0-17.7 14.3-32 32-32z", "0 0 320 512", "#008374", "26px", "12px");
  var iCal       = badge("M128 0c17.7 0 32 14.3 32 32V64H288V32c0-17.7 14.3-32 32-32s32 14.3 32 32V64h48c26.5 0 48 21.5 48 48v48H0V112C0 85.5 21.5 64 48 64H96V32c0-17.7 14.3-32 32-32zM0 192H448V464c0 26.5-21.5 48-48 48H48c-26.5 0-48-21.5-48-48V192zm64 80v32c0 8.8 7.2 16 16 16h32c8.8 0 16-7.2 16-16V272c0-8.8-7.2-16-16-16H80c-8.8 0-16 7.2-16 16zm128 0v32c0 8.8 7.2 16 16 16h32c8.8 0 16-7.2 16-16V272c0-8.8-7.2-16-16-16H208c-8.8 0-16 7.2-16 16zm144-16c-8.8 0-16 7.2-16 16v32c0 8.8 7.2 16 16 16h32c8.8 0 16-7.2 16-16V272c0-8.8-7.2-16-16-16H336zM64 400v32c0 8.8 7.2 16 16 16h32c8.8 0 16-7.2 16-16V400c0-8.8-7.2-16-16-16H80c-8.8 0-16 7.2-16 16zm144-16c-8.8 0-16 7.2-16 16v32c0 8.8 7.2 16 16 16h32c8.8 0 16-7.2 16-16V400c0-8.8-7.2-16-16-16H208zm112 16v32c0 8.8 7.2 16 16 16h32c8.8 0 16-7.2 16-16V400c0-8.8-7.2-16-16-16H336c-8.8 0-16 7.2-16 16z", "0 0 448 512", "transparent", "26px", "14px");  // used in section header - gets special style below
  var iHouse     = badge("M575.8 255.5c0 18-15 32.1-32 32.1h-32l.7 160.2c0 2.7-.2 5.4-.5 8.1V472c0 22.1-17.9 40-40 40H456c-1.1 0-2.2 0-3.3-.1c-1.4 .1-2.8 .1-4.2 .1H416 392c-22.1 0-40-17.9-40-40V448 384c0-17.7-14.3-32-32-32H256c-17.7 0-32 14.3-32 32v64 24c0 22.1-17.9 40-40 40H160 128.1c-1.5 0-3-.1-4.5-.2c-1.2 .1-2.4 .2-3.6 .2H104c-22.1 0-40-17.9-40-40V360c0-.9 0-1.9 .1-2.8V287.6H32c-18 0-32-14-32-32.1c0-9 3-17 10-24L266.4 8c7-7 15-8 22-8s15 2 21 7L564.8 231.5c8 7 12 15 11 24z", "0 0 576 512", "#0e2a47", "24px", "11px");
  var iClock     = badge("M256 0a256 256 0 1 1 0 512A256 256 0 1 1 256 0zM232 120V256c0 8 4 15.5 10.7 20l96 64c11 7.4 25.9 4.4 33.3-6.7s4.4-25.9-6.7-33.3L280 243.2V120c0-13.3-10.7-24-24-24s-24 10.7-24 24z", "0 0 512 512", "#0e2a47", "24px", "11px");
  var iWrench    = badge("M78.6 5C69.1-2.4 55.6-1.5 47 7L7 47c-8.5 8.5-9.4 22-2.1 31.6l80 104c4.5 5.9 11.6 9.4 19 9.4h54.1l109 109c-14.7 29-10 65.4 14.3 89.6l112 112c12.5 12.5 32.8 12.5 45.3 0l64-64c12.5-12.5 12.5-32.8 0-45.3l-112-112c-24.2-24.2-60.6-29-89.6-14.3l-109-109V104c0-7.5-3.5-14.5-9.4-19L78.6 5zM19.9 396.1C7.2 408.8 0 426.1 0 444.1C0 481.6 30.4 512 67.9 512c18 0 35.3-7.2 48-19.9L233.7 374.3c-7.8-20.9-9-43.6-3.6-65.1l-61.7-61.7L19.9 396.1zM512 144c0-10.5-1.1-20.7-3.2-30.5c-2.4-11.2-16.1-14.1-24.2-6l-63.9 63.9c-3 3-7.1 4.7-11.3 4.7H352c-8.8 0-16-7.2-16-16V102.6c0-4.2 1.7-8.3 4.7-11.3l63.9-63.9c8.1-8.1 5.2-21.8-6-24.2C388.7 1.1 378.5 0 368 0C288.5 0 224 64.5 224 144l0 .8 85.3 85.3c36-9.1 75.8 .5 104 28.7L429 274.5c49-23 83-72.8 83-130.5zM56 432a24 24 0 1 1 48 0 24 24 0 1 1 -48 0z", "0 0 512 512", "#0e2a47", "24px", "10px");
  var iRemaining = badge("M0 112.5V422.3c0 18 10.1 35 27 41.3c87 32.5 174 10.3 261-11.9c79.8-20.3 159.6-40.7 239.3-18.9c23 6.3 48.7-9.5 48.7-33.4V89.7c0-18-10.1-35-27-41.3C462 15.9 375 38.1 288 60.3C208.2 80.6 128.4 100.9 48.7 79.1C25.6 72.8 0 88.6 0 112.5zM288 352c-44.2 0-80-43-80-96s35.8-96 80-96s80 43 80 96s-35.8 96-80 96zM64 352c35.3 0 64 28.7 64 64H64V352zm64-208c0 35.3-28.7 64-64 64V144h64zM512 304v64H448c0-35.3 28.7-64 64-64zM448 96h64v64c-35.3 0-64-28.7-64-64z", "0 0 576 512", "#0e2a47", "24px", "10px");
  var iRemainingGrid = icon("M0 112.5V422.3c0 18 10.1 35 27 41.3c87 32.5 174 10.3 261-11.9c79.8-20.3 159.6-40.7 239.3-18.9c23 6.3 48.7-9.5 48.7-33.4V89.7c0-18-10.1-35-27-41.3C462 15.9 375 38.1 288 60.3C208.2 80.6 128.4 100.9 48.7 79.1C25.6 72.8 0 88.6 0 112.5zM288 352c-44.2 0-80-43-80-96s35.8-96 80-96s80 43 80 96s-35.8 96-80 96zM64 352c35.3 0 64 28.7 64 64H64V352zm64-208c0 35.3-28.7 64-64 64V144h64zM512 304v64H448c0-35.3 28.7-64 64-64zM448 96h64v64c-35.3 0-64-28.7-64-64z", "0 0 576 512", "#0e2a47", "20px");
  var iMoney     = badge("M0 112.5V422.3c0 18 10.1 35 27 41.3c87 32.5 174 10.3 261-11.9c79.8-20.3 159.6-40.7 239.3-18.9c23 6.3 48.7-9.5 48.7-33.4V89.7c0-18-10.1-35-27-41.3C462 15.9 375 38.1 288 60.3C208.2 80.6 128.4 100.9 48.7 79.1C25.6 72.8 0 88.6 0 112.5zM288 352c-44.2 0-80-43-80-96s35.8-96 80-96s80 43 80 96s-35.8 96-80 96zM64 352c35.3 0 64 28.7 64 64H64V352zm64-208c0 35.3-28.7 64-64 64V144h64zM512 304v64H448c0-35.3 28.7-64 64-64zM448 96h64v64c-35.3 0-64-28.7-64-64z", "0 0 576 512", "#28a745", "24px", "10px");
  var iScale     = badge("M384 32H512c17.7 0 32 14.3 32 32s-14.3 32-32 32H398.4c-5.2 25.8-22.9 47.1-46.4 57.3V448H512c17.7 0 32 14.3 32 32s-14.3 32-32 32H320 128c-17.7 0-32-14.3-32-32s14.3-32 32-32H288V153.3c-23.5-10.3-41.2-31.6-46.4-57.3H128c-17.7 0-32-14.3-32-32s14.3-32 32-32H256c14.6-19.4 37.8-32 64-32s49.4 12.6 64 32zm55.6 288H584.4L512 195.8 439.6 320zM512 416c-62.9 0-115.2-34-126-78.9c-2.6-11 1-22.3 6.7-32.1l95.2-163.2c5-8.6 14.2-13.8 24.1-13.8s19.1 5.3 24.1 13.8l95.2 163.2c5.7 9.8 9.3 21.1 6.7 32.1C627.2 382 574.9 416 512 416zM126.8 195.8L54.4 320H199.3L126.8 195.8zM.9 337.1c-2.6-11 1-22.3 6.7-32.1l95.2-163.2c5-8.6 14.2-13.8 24.1-13.8s19.1 5.3 24.1 13.8l95.2 163.2c5.7 9.8 9.3 21.1 6.7 32.1C242 382 189.7 416 126.8 416S11.7 382 .9 337.1z", "0 0 640 512", "#28a745", "24px", "10px");
  var iScaleRed  = badge("M384 32H512c17.7 0 32 14.3 32 32s-14.3 32-32 32H398.4c-5.2 25.8-22.9 47.1-46.4 57.3V448H512c17.7 0 32 14.3 32 32s-14.3 32-32 32H320 128c-17.7 0-32-14.3-32-32s14.3-32 32-32H288V153.3c-23.5-10.3-41.2-31.6-46.4-57.3H128c-17.7 0-32-14.3-32-32s14.3-32 32-32H256c14.6-19.4 37.8-32 64-32s49.4 12.6 64 32zm55.6 288H584.4L512 195.8 439.6 320zM512 416c-62.9 0-115.2-34-126-78.9c-2.6-11 1-22.3 6.7-32.1l95.2-163.2c5-8.6 14.2-13.8 24.1-13.8s19.1 5.3 24.1 13.8l95.2 163.2c5.7 9.8 9.3 21.1 6.7 32.1C627.2 382 574.9 416 512 416zM126.8 195.8L54.4 320H199.3L126.8 195.8zM.9 337.1c-2.6-11 1-22.3 6.7-32.1l95.2-163.2c5-8.6 14.2-13.8 24.1-13.8s19.1 5.3 24.1 13.8l95.2 163.2c5.7 9.8 9.3 21.1 6.7 32.1C242 382 189.7 416 126.8 416S11.7 382 .9 337.1z", "0 0 640 512", "#c82333", "24px", "10px");
  var iCalDet    = icon("M96 32V64H48C21.5 64 0 85.5 0 112v48H448V112c0-26.5-21.5-48-48-48H352V32c0-17.7-14.3-32-32-32s-32 14.3-32 32V64H160V32c0-17.7-14.3-32-32-32S96 14.3 96 32zM448 192H0V464c0 26.5 21.5 48 48 48H400c26.5 0 48-21.5 48-48V192z", "0 0 448 512", "#666666", "18px");
  var iCalW      = icon("M128 0c17.7 0 32 14.3 32 32V64H288V32c0-17.7 14.3-32 32-32s32 14.3 32 32V64h48c26.5 0 48 21.5 48 48v48H0V112C0 85.5 21.5 64 48 64H96V32c0-17.7 14.3-32 32-32zM0 192H448V464c0 26.5-21.5 48-48 48H48c-26.5 0-48-21.5-48-48V192zm80 64c-8.8 0-16 7.2-16 16v64c0 8.8 7.2 16 16 16H368c8.8 0 16-7.2 16-16V272c0-8.8-7.2-16-16-16H80z", "0 0 448 512", "#666666", "18px");
  var iCard      = icon("M64 32C28.7 32 0 60.7 0 96v32H576V96c0-35.3-28.7-64-64-64H64zM576 224H0V416c0 35.3 28.7 64 64 64H512c35.3 0 64-28.7 64-64V224zM112 352h64c8.8 0 16 7.2 16 16s-7.2 16-16 16H112c-8.8 0-16-7.2-16-16s7.2-16 16-16zm112 16c0-8.8 7.2-16 16-16H368c8.8 0 16 7.2 16 16s-7.2 16-16 16H240c-8.8 0-16-7.2-16-16z", "0 0 576 512", "#666666", "18px");
  var iClip      = icon("M192 0c-41.8 0-77.4 26.7-90.5 64H64C28.7 64 0 92.7 0 128V448c0 35.3 28.7 64 64 64H320c35.3 0 64-28.7 64-64V128c0-35.3-28.7-64-64-64H282.5C269.4 26.7 233.8 0 192 0zm0 64a32 32 0 1 1 0 64 32 32 0 1 1 0-64zM112 192H272c8.8 0 16 7.2 16 16s-7.2 16-16 16H112c-8.8 0-16-7.2-16-16s7.2-16 16-16z", "0 0 384 512", "#666666", "18px");
  var iCheck     = icon("M256 512A256 256 0 1 0 256 0a256 256 0 1 0 0 512zM369 209L241 337c-9.4 9.4-24.6 9.4-33.9 0l-64-64c-9.4-9.4-9.4-24.6 0-33.9s24.6-9.4 33.9 0l47 47L335 175c9.4-9.4 24.6-9.4 33.9 0s9.4 24.6 0 33.9z", "0 0 512 512", "#008374", "16px");
  var iCalGrid   = icon("M128 0c17.7 0 32 14.3 32 32V64H288V32c0-17.7 14.3-32 32-32s32 14.3 32 32V64h48c26.5 0 48 21.5 48 48v48H0V112C0 85.5 21.5 64 48 64H96V32c0-17.7 14.3-32 32-32zM0 192H448V464c0 26.5-21.5 48-48 48H48c-26.5 0-48-21.5-48-48V192zm64 80v32c0 8.8 7.2 16 16 16h32c8.8 0 16-7.2 16-16V272c0-8.8-7.2-16-16-16H80c-8.8 0-16 7.2-16 16zm128 0v32c0 8.8 7.2 16 16 16h32c8.8 0 16-7.2 16-16V272c0-8.8-7.2-16-16-16H208c-8.8 0-16 7.2-16 16zm144-16c-8.8 0-16 7.2-16 16v32c0 8.8 7.2 16 16 16h32c8.8 0 16-7.2 16-16V272c0-8.8-7.2-16-16-16H336zM64 400v32c0 8.8 7.2 16 16 16h32c8.8 0 16-7.2 16-16V400c0-8.8-7.2-16-16-16H80c-8.8 0-16 7.2-16 16zm144-16c-8.8 0-16 7.2-16 16v32c0 8.8 7.2 16 16 16h32c8.8 0 16-7.2 16-16V400c0-8.8-7.2-16-16-16H208zm112 16v32c0 8.8 7.2 16 16 16h32c8.8 0 16-7.2 16-16V400c0-8.8-7.2-16-16-16H336c-8.8 0-16 7.2-16 16z", "0 0 448 512", "#0e2a47", "20px");
  var iHouseGrid = icon("M575.8 255.5c0 18-15 32.1-32 32.1h-32l.7 160.2c0 2.7-.2 5.4-.5 8.1V472c0 22.1-17.9 40-40 40H456c-1.1 0-2.2 0-3.3-.1c-1.4 .1-2.8 .1-4.2 .1H416 392c-22.1 0-40-17.9-40-40V448 384c0-17.7-14.3-32-32-32H256c-17.7 0-32 14.3-32 32v64 24c0 22.1-17.9 40-40 40H160 128.1c-1.5 0-3-.1-4.5-.2c-1.2 .1-2.4 .2-3.6 .2H104c-22.1 0-40-17.9-40-40V360c0-.9 0-1.9 .1-2.8V287.6H32c-18 0-32-14-32-32.1c0-9 3-17 10-24L266.4 8c7-7 15-8 22-8s15 2 21 7L564.8 231.5c8 7 12 15 11 24z", "0 0 576 512", "#0e2a47", "20px");
  var iWrenchGrid= icon("M78.6 5C69.1-2.4 55.6-1.5 47 7L7 47c-8.5 8.5-9.4 22-2.1 31.6l80 104c4.5 5.9 11.6 9.4 19 9.4h54.1l109 109c-14.7 29-10 65.4 14.3 89.6l112 112c12.5 12.5 32.8 12.5 45.3 0l64-64c12.5-12.5 12.5-32.8 0-45.3l-112-112c-24.2-24.2-60.6-29-89.6-14.3l-109-109V104c0-7.5-3.5-14.5-9.4-19L78.6 5zM19.9 396.1C7.2 408.8 0 426.1 0 444.1C0 481.6 30.4 512 67.9 512c18 0 35.3-7.2 48-19.9L233.7 374.3c-7.8-20.9-9-43.6-3.6-65.1l-61.7-61.7L19.9 396.1zM512 144c0-10.5-1.1-20.7-3.2-30.5c-2.4-11.2-16.1-14.1-24.2-6l-63.9 63.9c-3 3-7.1 4.7-11.3 4.7H352c-8.8 0-16-7.2-16-16V102.6c0-4.2 1.7-8.3 4.7-11.3l63.9-63.9c8.1-8.1 5.2-21.8-6-24.2C388.7 1.1 378.5 0 368 0C288.5 0 224 64.5 224 144l0 .8 85.3 85.3c36-9.1 75.8 .5 104 28.7L429 274.5c49-23 83-72.8 83-130.5zM56 432a24 24 0 1 1 48 0 24 24 0 1 1 -48 0z", "0 0 512 512", "#0e2a47", "20px");
  var iHistory   = icon("M75 75L41 41C25.9 25.9 0 36.6 0 57.9V168c0 13.3 10.7 24 24 24H134.1c21.4 0 32.1-25.9 17-41l-30.8-30.8C155 85.5 203 64 256 64c106 0 192 86 192 192s-86 192-192 192c-40.8 0-78.6-12.7-109.7-34.4c-14.5-10.1-34.4-6.6-44.6 7.9s-6.6 34.4 7.9 44.6C151.2 495 201.7 512 256 512c141.4 0 256-114.6 256-256S397.4 0 256 0C185.3 0 121.3 28.7 75 75zm181 53c-13.3 0-24 10.7-24 24V256c0 6.4 2.5 12.5 7 17l72 72c9.4 9.4 24.6 9.4 33.9 0s9.4-24.6 0-33.9l-65-65V152c0-13.3-10.7-24-24-24z", "0 0 512 512", "#0e2a47", "20px");
  var iWallet    = icon("M64 32C28.7 32 0 60.7 0 96V416c0 35.3 28.7 64 64 64H448c35.3 0 64-28.7 64-64V192c0-35.3-28.7-64-64-64H80c-8.8 0-16-7.2-16-16s7.2-16 16-16H448c17.7 0 32-14.3 32-32s-14.3-32-32-32H64zM416 272a32 32 0 1 1 0 64 32 32 0 1 1 0-64z", "0 0 512 512", "#0e2a47", "20px");
  var iInfo      = icon("M256 512A256 256 0 1 0 256 0a256 256 0 1 0 0 512zM216 336h24V272H216c-13.3 0-24-10.7-24-24s10.7-24 24-24h48c13.3 0 24 10.7 24 24v88h8c13.3 0 24 10.7 24 24s-10.7 24-24 24H216c-13.3 0-24-10.7-24-24s10.7-24 24-24zm40-208a32 32 0 1 1 0 64 32 32 0 1 1 0-64z", "0 0 512 512", "#0e2a47", "14px");
  var iHeadset   = icon("M256 48C141.1 48 48 141.1 48 256v40c0 13.3-10.7 24-24 24s-24-10.7-24-24V256C0 114.6 114.6 0 256 0S512 114.6 512 256V400.1c0 48.6-39.4 88-88.1 88L313.6 488c-8.3 14.3-23.8 24-41.6 24H240c-26.5 0-48-21.5-48-48s21.5-48 48-48h32c17.8 0 33.3 9.7 41.6 24l110.4 .1c22.1 0 40-17.9 40-40V256c0-114.9-93.1-208-208-208zM144 208h16c17.7 0 32 14.3 32 32V352c0 17.7-14.3 32-32 32H144c-35.3 0-64-28.7-64-64V272c0-35.3 28.7-64 64-64zm224 0c35.3 0 64 28.7 64 64v48c0 35.3-28.7 64-64 64H352c-17.7 0-32-14.3-32-32V240c0-17.7 14.3-32 32-32h16z", "0 0 512 512", "#008374", "13px");
  var iPhone     = icon("M164.9 24.6c-7.7-18.6-28-28.5-47.4-23.2l-88 24C12.1 30.2 0 46 0 64C0 311.4 200.6 512 448 512c18 0 33.8-12.1 38.6-29.5l24-88c5.3-19.4-4.6-39.7-23.2-47.4l-96-40c-16.3-6.8-35.2-2.1-46.3 11.6L304.7 368C234.3 334.7 177.3 277.7 144 207.3L193.3 167c13.7-11.2 18.4-30 11.6-46.3l-40-96z", "0 0 512 512", "#0e2a47", "12px");
  var iEmail     = icon("M48 64C21.5 64 0 85.5 0 112c0 15.1 7.1 29.3 19.2 38.4L236.8 313.6c11.4 8.5 27 8.5 38.4 0L492.8 150.4c12.1-9.1 19.2-23.3 19.2-38.4c0-26.5-21.5-48-48-48H48zM0 176V384c0 35.3 28.7 64 64 64H448c35.3 0 64-28.7 64-64V176L294.4 339.2c-22.8 17.1-54 17.1-76.8 0L0 176z", "0 0 512 512", "#0e2a47", "12px");
  var iCalGray   = '<div style="background:transparent;width:26px;height:26px;border-radius:50%;display:flex;align-items:center;justify-content:center;flex-shrink:0;border:1px solid #e0e0e0;">' + icon("M128 0c17.7 0 32 14.3 32 32V64H288V32c0-17.7 14.3-32 32-32s32 14.3 32 32V64h48c26.5 0 48 21.5 48 48v48H0V112C0 85.5 21.5 64 48 64H96V32c0-17.7 14.3-32 32-32zM0 192H448V464c0 26.5-21.5 48-48 48H48c-26.5 0-48-21.5-48-48V192zm64 80v32c0 8.8 7.2 16 16 16h32c8.8 0 16-7.2 16-16V272c0-8.8-7.2-16-16-16H80c-8.8 0-16 7.2-16 16zm128 0v32c0 8.8 7.2 16 16 16h32c8.8 0 16-7.2 16-16V272c0-8.8-7.2-16-16-16H208c-8.8 0-16 7.2-16 16zm144-16c-8.8 0-16 7.2-16 16v32c0 8.8 7.2 16 16 16h32c8.8 0 16-7.2 16-16V272c0-8.8-7.2-16-16-16H336zM64 400v32c0 8.8 7.2 16 16 16h32c8.8 0 16-7.2 16-16V400c0-8.8-7.2-16-16-16H80c-8.8 0-16 7.2-16 16zm144-16c-8.8 0-16 7.2-16 16v32c0 8.8 7.2 16 16 16h32c8.8 0 16-7.2 16-16V400c0-8.8-7.2-16-16-16H208zm112 16v32c0 8.8 7.2 16 16 16h32c8.8 0 16-7.2 16-16V400c0-8.8-7.2-16-16-16H336c-8.8 0-16 7.2-16 16z", "0 0 448 512", "#666666", "14px") + '</div>';

  var page =
    '<div class="receipt-container">' +

    '<div class="sender-block">' +
    '<img class="sender-logo-icon" src="' + logoSrc + '">' +
    '<div class="sender-text-content"><strong>Gallarzo LLC</strong><br>2989 Pacific Vista<br>Chino Hills, CA 91709</div>' +
    '</div>' +

    '<div class="recipient-block">' +
    '<strong style="' + tenantNameStyle + '">' + escHtml(d.tenantName) + '</strong>' +
    escHtml(maddr1) + '<br>' + escHtml(maddr2) +
    '</div>' +

    '<div class="header-right-meta">' +
    '<div class="logo-area">' +
    '<img src="' + logoSrc + '" style="width:72px;height:72px;object-fit:contain;">' +
    '<div class="logo-text" style="display:flex;align-items:baseline;gap:8px;"><h1>GALLARZO</h1><h1 style="letter-spacing:1px;">LLC</h1></div>' +
    '</div>' +
    '<div class="receipt-title">RENTAL <span>RECEIPT</span></div>' +
    '</div>' +

    '<div class="content-wrapper">' +
    '<div class="content-columns">' +

    '<div class="column col-left">' +
    '<div class="section-header">' + iDollar + '<h3>ACCOUNT SUMMARY</h3></div>' +
    '<div class="' + rentSummaryClass + '"><div class="row-label">' + iHouse + '<div class="rent-line-text"><span>Rent Amount</span><small class="rent-address-line">' + rentAddressHtml + '</small></div></div><div class="row-value rent-row-value">' + fmtMoney(d.rentDue) + '</div></div>' +
    '<div class="summary-row"><div class="row-label">' + iClock + '<div><span>Late Fee</span><small>' + (d.isWaived ? 'Waived' : '(If applicable)') + '</small></div></div><div class="row-value' + (d.lateFeeRaw > 0 && !d.isWaived ? ' text-red' : d.isWaived ? ' text-green' : '') + '">' + (d.isWaived ? fmtMoney(d.lateFeeRaw) + ' <span style="font-size:10px;font-weight:600;color:var(--green);">(Waived)</span>' : fmtMoney(d.lateFee)) + '</div></div>' +
    '<div class="summary-row"><div class="row-label">' + iWrench + '<div><span>Other Charges</span><small>(If applicable)</small></div></div><div class="row-value">' + fmtMoney(d.extra) + '</div></div>' +
    '<div class="summary-row"><div class="row-label">' + iRemaining + '<div><span>Remaining Balance</span><small>(Previous unpaid)</small></div></div><div class="row-value' + (d.priorBalance > 0 ? ' text-red' : '') + '">' + fmtMoney(d.priorBalance > 0 ? d.priorBalance : 0) + '</div></div>' +
    '<div class="totals-section">' +
    '<div class="total-row"><span>TOTAL AMOUNT DUE</span><span>' + fmtMoney(d.totalDue) + '</span></div>' +
    '<div class="total-row amount-paid-box"><div class="row-label">' + iMoney + '<span class="text-green">AMOUNT PAID</span></div><span class="text-green">-' + fmtMoney(d.amountPaid) + '</span></div>' +
    '<div class="total-row ' + balanceBox + '"><div class="row-label">' + (hasBalance ? iScaleRed : iScale) + '<span class="' + balanceClass + '">' + balanceLabel + '</span></div><span class="' + balanceClass + '">' + balanceDisplay + '</span></div>' +
    '</div></div>' +

    '<div class="column col-right">' +
    '<div class="section-header">' + iCalGray + '<h3 style="color:var(--text-dark);">PAYMENT DETAILS</h3></div>' +
    '<div class="detail-item">' + iCalDet + '<div class="detail-text"><label>Date Paid</label><p>' + escHtml(d.paymentDate) + '</p></div></div>' +
    '<div class="detail-item">' + iCalW + '<div class="detail-text"><label>Lease Period</label><p>' + escHtml(leasePeriod) + '</p></div></div>' +
    '<div class="detail-item">' + iCard + '<div class="detail-text"><label>Payment Method</label><p>' + escHtml(d.paymentMethod) + '</p></div></div>' +
    '<div class="detail-item">' + iClip + '<div class="detail-text"><label>Notes</label><p style="font-weight:400;color:var(--text-gray);font-size:12px;margin-top:4px;">' + escHtml(notes) + '</p></div></div>' +
    '</div>' +
    '</div>' +

    '<div class="receipt-confirmation">' + iCheck + '<span>Thank you! Your payment has been received.</span></div>' +

    '<div class="next-month-title">NEXT MONTH PAYMENT</div>' +
    '<div class="next-month-box"><div class="next-month-grid">' +
    '<div class="grid-item">' + iCalGrid + '<h4>Due Date</h4><p>' + escHtml(d.nextDueDate) + '</p></div>' +
    '<div class="grid-item">' + iHouseGrid + '<h4>Rent Amount</h4><p>' + fmtMoney(d.nextRent) + '</p></div>' +
    '<div class="grid-item">' + iWrenchGrid + '<h4>Extra Charges</h4><p>' + fmtMoney(d.nextExtra) + '</p></div>' +
    '<div class="grid-item">' + iRemainingGrid + '<h4>' + nextBalanceLabel + '</h4><p class="' + nextBalanceClass + '">' + nextBalanceDisplay + '</p></div>' +
    '<div class="grid-item total-due">' + iWallet + '<h4>TOTAL DUE</h4><p>' + fmtMoney(nextTotalDisplay) + '</p></div>' +
    '</div>' +
    '<div class="late-fee-alert">' + iInfo + '<span>Payments received after ' + escHtml(d.lateDueDate) + ' will incur a late fee of ' + fmtMoney(d.nextLateFee) + '.</span></div>' +
    '</div>' +

    '<div class="footer">' +
    '<div class="footer-line" style="align-items:flex-start;"><div class="footer-support-icon" style="margin-top:1px;">' + iHeadset + '</div><span>We appreciate your on-time payment. If you have any questions, please contact us.</span></div>' +
    '<div class="footer-line footer-links"><span class="footer-link">' + iPhone + ' (951) 453-1632</span><span class="footer-link">' + iEmail + ' gallarzollc@gmail.com</span></div>' +
    '</div>' +
    '</div>' +
    '</div>';

  if (pageOnly) return page;
  return '<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8">' + getReceiptCss() + '</head><body>' + page + '</body></html>';
}

// =============================================================
//  SIDEBAR HTML
// =============================================================
function buildSidebarHtml() {
  var html = '<!DOCTYPE html><html><head><meta charset="UTF-8">';
  html += '<style>';
  html += 'body{font-family:Arial,sans-serif;font-size:13px;padding:16px;color:#333;}';
  html += 'h2{color:#0e2a47;margin-bottom:16px;font-size:16px;}';
  html += 'label{display:block;font-weight:bold;margin-bottom:4px;color:#0e2a47;}';
  html += 'select{width:100%;padding:7px;border:1px solid #ccc;border-radius:4px;font-size:13px;margin-bottom:14px;}';
  html += 'button{width:100%;padding:10px;background:#008374;color:white;border:none;border-radius:4px;font-size:14px;font-weight:bold;cursor:pointer;margin-bottom:10px;}';
  html += 'button:hover{background:#006b5e;}button:disabled{background:#aaa;cursor:not-allowed;}';
  html += '.status{padding:10px;border-radius:4px;font-size:12px;display:none;margin-top:8px;white-space:pre-wrap;}';
  html += '.ok{background:#eef7f6;color:#005f52;border:1px solid #008374;}';
  html += '.err{background:#fdecea;color:#a00;border:1px solid #c82333;}';
  html += '.warn{background:#fff8e1;color:#7a5800;border:1px solid #f0c000;}';
  html += '.spinner{display:none;text-align:center;padding:10px;color:#666;}';
  html += '</style></head><body>';
  html += '<h2>&#127968; Generate Receipts</h2>';
  html += '<label for="monthSel">Rental Month</label>';
  html += '<select id="monthSel"><option value="">Loading months...</option></select>';
  html += '<label for="tenantSel">Tenant</label>';
  html += '<select id="tenantSel"><option value="">Select a month first</option></select>';
  html += '<button id="genBtn" onclick="startGenerate()" disabled>Generate Receipts</button>';
  html += '<div class="spinner" id="spinner">Working...</div>';
  html += '<div class="status" id="status"></div>';
  html += '<script>';
  html += 'google.script.run';
  html += '  .withSuccessHandler(function(months){';
  html += '    var sel = document.getElementById("monthSel");';
  html += '    sel.innerHTML = "";';
  html += '    var o = document.createElement("option"); o.value = ""; o.text = "-- Select Month --"; sel.appendChild(o);';
  html += '    months.forEach(function(m){ var op = document.createElement("option"); op.value = m; op.text = m; sel.appendChild(op); });';
  html += '  })';
  html += '  .withFailureHandler(function(e){';
  html += '    var sel = document.getElementById("monthSel"); sel.innerHTML = "";';
  html += '    var o = document.createElement("option"); o.text = "Error: " + e.message; sel.appendChild(o);';
  html += '  })';
  html += '  .getAvailableMonths();';
  html += 'document.getElementById("monthSel").addEventListener("change", function(){';
  html += '  var month = this.value, tSel = document.getElementById("tenantSel"), btn = document.getElementById("genBtn");';
  html += '  btn.disabled = true; tSel.innerHTML = "";';
  html += '  if (!month) { var o = document.createElement("option"); o.text = "Select a month first"; tSel.appendChild(o); return; }';
  html += '  var lo = document.createElement("option"); lo.text = "Loading..."; tSel.appendChild(lo);';
  html += '  google.script.run';
  html += '    .withSuccessHandler(function(tenants){';
  html += '      tSel.innerHTML = "";';
  html += '      var a = document.createElement("option"); a.value = "all"; a.text = "-- All tenants --"; tSel.appendChild(a);';
  html += '      tenants.forEach(function(t){ var op = document.createElement("option"); op.value = t; op.text = t; tSel.appendChild(op); });';
  html += '      btn.disabled = false;';
  html += '    })';
  html += '    .withFailureHandler(function(e){ tSel.innerHTML = ""; var o = document.createElement("option"); o.text = "Error: " + e.message; tSel.appendChild(o); })';
  html += '    .getTenantsForMonth(month);';
  html += '});';
  html += 'function startGenerate() {';
  html += '  var month = document.getElementById("monthSel").value;';
  html += '  var tenant = document.getElementById("tenantSel").value;';
  html += '  if (!month) { alert("Please select a month."); return; }';
  html += '  var mode = (tenant === "all") ? "all" : "single";';
  html += '  document.getElementById("genBtn").disabled = true;';
  html += '  document.getElementById("spinner").style.display = "block";';
  html += '  document.getElementById("status").style.display = "none";';
  html += '  google.script.run';
  html += '    .withSuccessHandler(function(r){';
  html += '      if (!r.success) { setLoading(false); showStatus("Error: " + r.message, "err"); return; }';
  html += '      showStatus("Queue ready: " + r.total + " receipt(s).", "warn");';
  html += '      runBatch();';
  html += '    })';
  html += '    .withFailureHandler(function(e){ setLoading(false); showStatus("Error: " + e.message, "err"); })';
  html += '    .initQueue(mode, month, tenant);';
  html += '}';
  html += 'function runBatch() {';
  html += '  google.script.run';
  html += '    .withSuccessHandler(function(r){';
  html += '      showStatus(r.message, r.done ? "ok" : "warn");';
  html += '      if (!r.done) { setTimeout(runBatch, 1500); } else { setLoading(false); }';
  html += '    })';
  html += '    .withFailureHandler(function(e){ setLoading(false); showStatus("Error: " + e.message, "err"); })';
  html += '    .processBatch();';
  html += '}';
  html += 'function setLoading(on) {';
  html += '  document.getElementById("spinner").style.display = on ? "block" : "none";';
  html += '  document.getElementById("genBtn").disabled = on;';
  html += '}';
  html += 'function showStatus(msg, cls) {';
  html += '  var el = document.getElementById("status");';
  html += '  el.textContent = msg; el.className = "status " + cls; el.style.display = "block";';
  html += '}';
  html += '<\/script></body></html>';
  return html;
}


// =============================================================
//  DIAGNOSE
// =============================================================
function diagnose() {
  var msg = "DIAGNOSE\n";
  try {
    var ss = SpreadsheetApp.getActiveSpreadsheet();
    msg += "Spreadsheet: " + ss.getName() + "\n";
    var months = getAvailableMonths();
    msg += "Months: " + months.join(", ") + "\n";
    for (var i = 0; i < months.length; i++) {
      var t = getTenantsForMonth(months[i]);
      msg += "  " + months[i] + " (" + t.length + "): " + t.join(", ") + "\n";
    }
    var rootId = extractId(DRIVE_ROOT_FOLDER_URL);
    msg += "Folder: " + DriveApp.getFolderById(rootId).getName();
  } catch(e) {
    msg += "ERROR: " + e.message;
  }
  Logger.log(msg);
  SpreadsheetApp.getUi().alert(msg);
}


// =============================================================
//  HELPERS
// =============================================================
function extractId(u) {
  if (!u) return "";
  var m = u.match(/\/folders\/([a-zA-Z0-9_-]+)/);
  if (m) return m[1];
  m = u.match(/\/d\/([a-zA-Z0-9_-]+)/);
  if (m) return m[1];
  return u.trim();
}
function getOrCreateFolder(parent, name) {
  var it = parent.getFoldersByName(name);
  return it.hasNext() ? it.next() : parent.createFolder(name);
}
function sanitizeName(s) {
  return s.replace(/[\/\\:*?"<>|]/g, "_").trim();
}
function toNum(val) {
  if (val === null || val === undefined || val === "") return 0;
  var n = parseFloat(String(val).replace(/[$,]/g, ""));
  return isNaN(n) ? 0 : n;
}
function roundMoney(n) {
  return Math.round((toNum(n) + Number.EPSILON) * 100) / 100;
}
function recipientNameStyle(name) {
  // Keep tenant names on one line, but keep the normal 10pt size used by the address block.
  // The wider recipient block prevents normal long names from wrapping.
  // Only extremely long names shrink slightly, then ellipsis is used as a safety fallback.
  var len = String(name || "").length;
  var fontSize = 10;
  if (len > 80) fontSize = 8.4;
  else if (len > 70) fontSize = 8.8;
  else if (len > 60) fontSize = 9.2;
  else if (len > 52) fontSize = 9.6;
  return "display:block;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;font-size:" + fontSize + "pt;line-height:1.12;margin:0;padding:0;";
}
function fmtMoney(n) {
  if (!n || n === 0) return "$0.00";
  var s = n < 0 ? "-" : "";
  return s + "$" + Math.abs(n).toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",");
}
function fmtDate(val) {
  if (!val) return "";
  var d = (val instanceof Date) ? val : new Date(val);
  if (isNaN(d)) return String(val);
  return String(d.getMonth()+1).padStart(2,"0") + "/" + String(d.getDate()).padStart(2,"0") + "/" + d.getFullYear();
}
function lastDayOfMonth(monthName, year) {
  var m = MONTH_ORDER[monthName];
  if (!m) return 30;
  return new Date(year, m, 0).getDate();
}
function escHtml(s) {
  return String(s || "").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;");
}
